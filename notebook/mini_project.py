...
# 입력된 X 데이터.
st.header("입력된 X 데이터:")
my_X_raw = np.array([[my_cylinders,my_displacement,my_horsepower,my_weight, my_acceleration, my_model_year, japan, usa]])
my_df_X_raw = pd.DataFrame(data=my_X_raw, columns=['cylinders','displacement','horsepower','weight', 'acceleration', 'model_year', 'japan','usa'])

st.write(my_df_X_raw)


# 전처리된 X 데이터.
with open("C:/Users/oocheol/This_is_Code_Test/notebook/data/my_scaler.pkl","rb") as f:
    my_scaler = pickle.load(f)
my_X_scaled = my_scaler.transform(my_X_raw)     # fit_transform이 아닌 transform!!

st.header("전처리된 X 데이터:")
my_df_X_scaled = pd.DataFrame(data=my_X_scaled, columns=['cylinders','displacement','horsepower','weight', 'acceleration', 'model_year', 'japan','usa'])
st.write(my_df_X_scaled)

# 예측.
with open("C:/Users/oocheol/This_is_Code_Test/notebook/data/my_regressor.pkl","rb") as f:
