...
def post_message(message):
    channel = '#stock'
    token = "xoxb-2298822265414-2298830743254-Np1WFxIYqYhvNaHhtV9SftYj"
...
def get_stock_balance(code):
    ...
    if code == 'ALL':
        post_message('계좌명: ' + str(cpBalance.GetHeaderValue(0)))
        post_message('결제잔고수량 : ' + str(cpBalance.GetHeaderValue(1)))
        post_message('평가금액: ' + str(cpBalance.GetHeaderValue(3)))
        post_message('평가손익: ' + str(cpBalance.GetHeaderValue(4)))
        post_message('종목수: ' + str(cpBalance.GetHeaderValue(7)))
    stocks = []
    for i in range(cpBalance.GetHeaderValue(7)):
        ...
        if code == 'ALL':
            post_message(str(i+1) + ' ' + stock_code + '(' + stock_name + ')' 
                + ':' + str(stock_qty))
...
def get_target_price(code):
    ...
    try:
        ...
        post_message("`get_target_price() -> exception! " + str(ex) + "`")
        return None
...
def get_movingaverage(code, window):
    ...
    try:
        ...
        post_message('get_movingavrg(' + str(window) + ') -> exception! ' + str(ex))
        return None    
...
def sell_all():
    ...
    try:
        ...
        post_message("sell_all() -> exception! " + str(ex))

if __name__ == '__main__': 
    try:
        symbol_list = ['A019550','A002630','A002710','A011200','A007860', 
                        'A050890', 'A069920', 'A142280', 'A048260']
        ...
        while True:
            ...
            if t_sell < t_now < t_exit:  # PM 03:15 ~ PM 03:20 : 일괄 매도
                if sell_all() == True:
                    post_message('`sell_all() returned True -> self-destructed!`')
                    sys.exit(0)
            if t_exit < t_now:  # PM 03:20 ~ :프로그램 종료
                post_message('`self-destructed!`')
                sys.exit(0)
            time.sleep(3)
    except Exception as ex:
        post_message('`main -> exception! ' + str(ex) + '`')